...
&emsp;&emsp;这里要知道的是q-开头的属性指令是框架约定的，不同的框架约定的不一样，例如ng-、v-、ms-，这些大家也都见过或用过。这里viewModel创建进行绑定的原理就这么简单，按照这个思路去扩充，就可以自己写一个mvvm框架。当然完整的框架涉及东西多的多，含有丰富的directive、filter、表达式、vm完善的api和甚至一些兼容性处理等。
